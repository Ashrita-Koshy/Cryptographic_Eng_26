#ifndef NTT_H
#define NTT_H

#include <stdint.h>
#include <stddef.h>
#include "config.h"
#include <stddef.h>

/* Precomputed Zeta powers as defined in FIPS 203 Appendix A */
static const int16_t zetaPowers[128] = {
    1,    1729, 2580, 3289, 2642,  630, 1897,  848,
    1062, 1919,  193,  797, 2786, 3260,  569, 1746,
     296, 2447, 1339, 1476, 3046,   56, 2240, 1333,
    1426, 2094,  535, 2882, 2393, 2879, 1974,  821,
     289,  331, 3253, 1756, 1197, 2304, 2277, 2055,
     650, 1977, 2513,  632, 2865,   33, 1320, 1915,
    2319, 1435,  807,  452, 1438, 2868, 1534, 2402,
    2647, 2617, 1481,  648, 2474, 3110, 1227,  910,
      17, 2761,  583, 2649, 1637,  723, 2288, 1100,
    1409, 2662, 3281,  233,  756, 2156, 3015, 3050,
    1703, 1651, 2789, 1789, 1847,  952, 1461, 2687,
     939, 2308, 2437, 2388,  733, 2337,  268,  641,
    1584, 2298, 2037, 3220,  375, 2549, 2090, 1645,
    1063,  319, 2773,  757, 2099,  561, 2466, 2594,
    2804, 1092,  403, 1026, 1143, 2150, 2775,  886,
    1722, 1212, 1874, 1029, 2110, 2935,  885, 2154
};

/* Precomputed Zeta odd powers as defined in FIPS 203 Appendix A */
static const int16_t zetaOddPowers[128] = {
      17,   -17, 2761, -2761,  583,  -583, 2649, -2649,
    1637, -1637,  723,  -723, 2288, -2288, 1100, -1100,
    1409, -1409, 2662, -2662, 3281, -3281,  233,  -233,
     756,  -756, 2156, -2156, 3015, -3015, 3050, -3050,
    1703, -1703, 1651, -1651, 2789, -2789, 1789, -1789,
    1847, -1847,  952,  -952, 1461, -1461, 2687, -2687,
     939,  -939, 2308, -2308, 2437, -2437, 2388, -2388,
     733,  -733, 2337, -2337,  268,  -268,  641,  -641,
    1584, -1584, 2298, -2298, 2037, -2037, 3220, -3220,
     375,  -375, 2549, -2549, 2090, -2090, 1645, -1645,
    1063, -1063,  319,  -319, 2773, -2773,  757,  -757,
    2099, -2099,  561,  -561, 2466, -2466, 2594, -2594,
    2804, -2804, 1092, -1092,  403,  -403, 1026, -1026,
    1143, -1143, 2150, -2150, 2775, -2775,  886,  -886,
    1722, -1722, 1212, -1212, 1874, -1874, 1029, -1029,
    2110, -2110, 2935, -2935,  885,  -885, 2154, -2154
};

//void ntt(uint16_t* f);

#pragma FUNC_ALWAYS_INLINE(ntt)
static inline void ntt(uint16_t *f) {
    size_t i = 1;
    uint32_t len, start, j;
    for (len = 128; len >= 2; len /= 2) {
        for (start = 0; start < MLKEM_N; start += 2 * len) {
            int32_t zeta = (int32_t)zetaPowers[i];
            i++;
            for (j = start; j < start + len; j++) {
                int32_t t = (zeta * (int32_t)f[j + len]) % MLKEM_Q;
                f[j + len] = (uint16_t)(((int32_t)f[j] - t + MLKEM_Q) % MLKEM_Q);
                f[j] = (uint16_t)(((int32_t)f[j] + t) % MLKEM_Q);
            }
        }
    }
}

#pragma FUNC_ALWAYS_INLINE(nttInverse)
static inline void nttInverse(uint16_t *f) {
    size_t i = 127;
    uint32_t len, start, j;
    for (len = 2; len <= 128; len *= 2) {
        for (start = 0; start < MLKEM_N; start += 2 * len) {
            int32_t zeta = (int32_t)zetaPowers[i];
            i--;
            for (j = start; j < start + len; j++) {
                int32_t t = (int32_t)f[j];
                f[j] = (uint16_t)(((int32_t)f[j + len] + t) % MLKEM_Q);
                f[j + len] = (uint16_t)((zeta * ((int32_t)f[j + len] - t + MLKEM_Q)) % MLKEM_Q);
            }
        }
    }
    for (i = 0; i < MLKEM_N; i++) {
        f[i] = (uint16_t)(((int32_t)f[i] * 3303) % MLKEM_Q);
    }
}

#pragma FUNC_ALWAYS_INLINE(multiplyNTT)
static inline void multiplyNTT(uint16_t *h, const uint16_t *f, const uint16_t *g) {
    uint16_t i;
    for (i = 0; i < 128; i++) {
        int32_t temp = ((int32_t)f[2*i + 1] * g[2*i + 1]) % MLKEM_Q;
        int32_t tz = (temp * zetaOddPowers[i]) % MLKEM_Q;
        if (tz < 0) tz += MLKEM_Q;
        h[2*i]     = (uint16_t)((((int32_t)f[2*i] * g[2*i]) % MLKEM_Q + tz) % MLKEM_Q);
        h[2*i + 1] = (uint16_t)(((((int32_t)f[2*i] * g[2*i + 1]) % MLKEM_Q) + (((int32_t)f[2*i + 1] * g[2*i]) % MLKEM_Q)) % MLKEM_Q);
    }
}

#endif
